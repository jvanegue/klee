...
--search=nurs:covnew \
--use-batching-search --batch-instructions=10000 --symbolic-strlen=0"

BUSYBOX_BC="./busybox_unstripped.bc"
APPLETS_LIST="acpid, add-shell, arping, arp, awk, base64, bzip2, bunzip2, bzcat, cal, ncal, date, chat, cmp, cksum,\
dc, diff, comm, cut, egrep, expand, expr"
#APPLET_COMMON_OPTS="--sym-stdin 8 --sym-stdout"
APPLET_COMMON_OPTS="--sym-stdout"

if [ $# -eq 0 ]
then
  echo " Usage: $0 APPLETNAME"
  echo ""
  echo " Available applets: $APPLETS_LIST"
  exit 1
fi
...
case "$APPLET" in
  ...
  base64)
    APPLET_OPTS="--sym-args 0 2 4 - --sym-stdin 30"
    ;;

  awk)
    APPLET_OPTS="--sym-args 0 4 5 --sym-stdin 30"
    ;;
  ...
  bzip2|bunzip2|bzcat)
    APPLET_OPTS="--sym-args 0 1 4 --sym-stdin 30"
    ;;
